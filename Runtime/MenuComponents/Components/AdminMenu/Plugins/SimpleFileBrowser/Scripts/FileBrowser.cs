...
namespace SimpleFileBrowser
{
	public class FileBrowser : MonoBehaviour, IListViewAdapter
	{
        ...
        public static Transform FileBrowserParent;
		...
		private static FileBrowser Instance
		{
			get
			{
				if( m_instance == null )
				{
					m_instance = Instantiate( Resources.Load<GameObject>( "SimpleFileBrowserCanvas" ), FileBrowserParent).GetComponent<FileBrowser>();
...
#pragma warning restore 0414
...
#else
			DEFAULT_PATH = Environment.GetFolderPath( Environment.SpecialFolder.Desktop );
#endif
		...
		private int CalculateLengthOfDropdownText( string str )
		{
			...
			myFont.RequestCharactersInTexture( str, (int)filterItemTemplate.fontSize,(FontStyle)filterItemTemplate.fontStyle );

			for( int i = 0; i < str.Length; i++ )
			{
				if( !myFont.GetCharacterInfo( str[i], out characterInfo, (int) filterItemTemplate.fontSize ) )
					totalLength += 5;
